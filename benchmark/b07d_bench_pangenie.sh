...
FINAL=/lustre1/g/paed_yangwl/snakesch/tmp/test_svhet/pangenie/$sample-sv-annot.bcf

OUT="/lustre1/g/paed_yangwl/snakesch/tmp/test_svhet/pangenie/truvari"

tmpdir="$(mktemp -d)"; trap 'rm -rf "$tmpdir"' EXIT

## TODO: Focus on deletions ?
bcftools +fill-tags $FINAL -- -t INFO/END | bcftools filter -i '(SVTYPE="DEL" & FMT/GT = "het")' -Wtbi -Oz -o $tmpdir/$sample-final.vcf.gz
